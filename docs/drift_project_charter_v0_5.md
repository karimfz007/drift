# THE FIRST NIGHT — Project Charter & Design Document *(codename DRIFT)*

**Version:** 0.5 (foundation + engineering + experience + the 3D pivot + the social economy)
*(v0.2, 2026-07-22 — library-first audit: Phaser 4 in §II.5; §II.10 engineering addenda. v0.3, 2026-07-22 — title **The First Night** (D-015); §I.9, §I.17, §I.18 after the human-enjoyment audit (D-016 … D-018). v0.4, 2026-07-22 — **the 3D pivot**: at Cycle 01's third gate the director overturned the 2D top-down direction; the game is now a low-poly 3D, close-third-person survival experience with Rust-like gameplay as the bar. §II.5 stack rewritten (Babylon.js), §I.7 building and §II.7 art updated. The brain, the pillars, the arc, the constitutions, and the pipeline are unchanged. Rationale and triage: D-027 … D-030. v0.5, 2026-07-24 — **the social economy** ratified as a fifth pillar, §I.2: a living economy between players (knowledge transfer, trade, help) arriving as an unfolding on top of the single-player-first build order, never a rewrite; its first law (constitutional, D-011 extended) bans offline harm from other players in every default mode. Director-confirmed; C1's constitutional pen. Rationale: D-054.)*
...
**Title:** The First Night *(named 2026-07-22, D-015; DRIFT remains the internal codename for files and the repo)*
...
**The social economy** *(v0.5 — director, 2026-07-24)*: the game's destiny includes a living economy between players — knowledge that moves (teaching, with transfer scaling on the teacher's experience and social skill; watching; found traces of other survivors), goods that trade, and help that travels. This pillar is key to the game's growth. Build order is unchanged: single-player-first remains the law of construction, and every system from v0.5 onward is designed multiplayer-shaped — data models that can serve many units — so the social layer arrives as an unfolding, not a rewrite.

**First law of the social economy (constitutional; D-011 extended):** the social layer never taxes absence. No player's build, progress, or stored goods can be meaningfully harmed by other players while they are away, in any default mode — offline raiding is constitutionally banned. Benign offline interaction (gifts, trades left waiting, notes, traces) is welcome and is morning-report material. Consensual opt-in modes may vary by explicit choice. The game whose promise is "you can leave" extends that promise against people, not only against the island.
...
- **Building — Rust mechanics, in world-space 3D** *(v0.4 — was 2D top-down tiles)*: pieces placed on a grid footprint in the world, visibly standing on the island. Tiered structures (e.g., branch/leaf → wood → stone → reinforced), placed on a grid footprint, with **matched-material upkeep** drawn from a base stockpile and **decay** when upkeep lapses or a structure is battered. Key fixtures: **fire** (warmth, cooking, light, deterrence) and a **bed/sleeping bag** (respawn anchor). Upkeep/decay in single-player serves the *maintenance-and-threat* loop (a reason to return and tend the base) and is `[TUNE]`/mercy-adjustable; it tightens when multiplayer arrives.
...
**Anti-grind (design intent — validated in playtest, D-016).** The output formula makes idle spam slow and active skillful play rich, but a formula alone does not prevent optimal repetition. The governing principle, tested cycle by cycle: **a mastered action becomes faster, richer, safer, or automatable — never the same chore with a larger number.** XP favors meaningful outcomes, novelty, and difficulty over raw repetition; solved chores become automatable in Act 2. Idle chopping for an hour stays a legitimate low-effort choice, not an exploit.
...
- **Rust** — the building/upkeep/decay economy, the offline-stakes tension (base fights while you sleep), and *(v0.4)* the gameplay bar for the 3D experience: free movement, physical gathering, spatial construction, a base that exists in the world. Our version: low-poly, single-player-first, mobile.
...
- **Terraria / The Bonfire** — proof that dig + build + survival works on mobile (our 2D-era references; the C01 prototype validated the loop's bones). **Muck / Valheim** *(v0.4)* — the low-poly 3D survival art bar: readable, stylized, performant.
...
- **The psychology (build-for-humans, rebalanced v0.3):** the primary engine is **competence, autonomy, and curiosity** — the player feels capable, chooses their approach, and is pulled forward by questions the island raises. Anticipation, loss aversion, scarcity, and the endowment effect are real and powerful — they are why the morning report works — but they are seasoning, not the meal: **guaranteed progress forms the base, uncertain rewards sit on top, and loss is always attributable, proportional, and recoverable** (D-011). The arc is **vulnerability → capability → earned command, with residual risk** — the player becomes highly capable, never immune; total domination would kill the tension that makes escaping worth it.

## 18. The Experience Constitution *(v0.3)*

Seven binding rules for what the game must feel like, adopted from the human-enjoyment audit (D-016) and compressed. They govern active play the way the offline fairness rules (§II.10, D-011) govern absence.

1. **Every action is legible.** Target, input, progress, resistance, and result are always visible — and audible. Feedback clarifies cause and effect; it never obscures the grounded tone.
2. **Challenge means decisions, not bigger numbers.** Threats are telegraphed; there are always at least two viable responses with readable tradeoffs. Difficulty escalates by introducing new decisions and combining learned systems before numerical margins ever tighten. A costly failure must pass the fair-challenge test: telegraphed, understandable, preventable, explainable afterward, instructive, and proportionally recoverable.
3. **Failure teaches; recovery preserves momentum.** The player can always explain what went wrong and knows a credible adjustment to try next.
4. **Tension earns relief.** Every crisis resolves into a safe beat — the lit fire is the archetype: warm light, visible recovery, a moment to plan.
5. **Three goal horizons stay visible:** right now, this session, and escape. Curiosity is a system, not just a setting — every phase must create at least one unanswered question, one obtainable clue, and one newly visible possibility.
6. **Progress changes how you play,** not just percentages (see §9's anti-grind principle). A mechanic that produces no decision, emotion, expression, or progression is removed.
7. **Respect the player's time and body.** The primary player is a busy adult on a phone — a survival and base-building fan, not assumed a genre expert, with 2–5 minute check-ins and occasional 15–30 minute expeditions; one-thumb readable, deep without menu friction, demanding without requiring attendance. Accessibility baseline: no information by color alone, scalable text and large touch targets, tap alternatives to holds where reasonable, motion/shake controls, audio cues mirrored visually, left/right control placement, and difficulty kept separate from accessibility. Hardcore veterans are served by explicit optional rules, never by making the default opaque or punitive. Honest systems (§8) always apply.
...
- **Engine: Babylon.js + TypeScript, web-first** *(v0.4 — was Phaser 4; D-028)*. Pinned to the current stable release at scaffold (9.x as of this amendment; exact pin recorded in `DEPENDENCIES.md`), upgraded only between cycles. Apache-2.0, TypeScript-native, batteries included: official Havok WASM physics, first-class glTF, WebGL2 baseline with a WebGPU path. Chosen because it keeps the brain byte-identical, keeps the entire proven pipeline (GitHub Pages, tap-a-link playtests, the smoke harness, CI), and is the deepest 3D toolkit in the crew's strongest language. Rejected: Unity (editor-centric — poor fit for an AI crew and a non-technical director's machine; weak mobile-web), Three.js (lower-level, more glue for the same result), PlayCanvas (cloud-editor dependency), react-three-fiber (D-009's concept-count rule).
- **The native escape hatch: Godot 4.x** *(v0.4)*. Godot's 3D web export is Compatibility-renderer-only with 30–100 MB payloads — wrong for phone browsers — but its native Android path is excellent. If mobile-web 3D hits a hard performance ceiling that low-poly discipline and optimization cycles cannot clear, the brain ports and Godot builds the APK. That gate, and only that gate, reopens this decision.
- **Perspective: low-poly 3D, close third-person** *(v0.4 — was 2D top-down; D-029)*, first-person as a later option; underground and underwater remain the vertical layers for mining and diving. Touch model: left stick to move, drag to look, tap the world to interact.
- **The architectural rule (non-negotiable, unchanged):** the game's **brain** — simulation, data model, reconciliation rules — lives in **plain TypeScript modules with zero rendering-engine imports** (mechanically enforced in CI; the forbidden import is now Babylon). The renderer only *draws.* Cycle 01→02 is this rule's proof: the same brain, tests untouched, under a new body.
- **Native** (App Store / Play Store) is a **launch-phase concern, deferred** — reached later via a wrapper or the Godot hatch above.
...
- **Target style: low-poly stylized 3D** *(v0.4)* — the Muck/Valheim readability bar, not Rust fidelity; grounded palette and lighting carry the mood. CC0-first assets (Kenney, Quaternius) until the Art Director hat activates. Rust sets the gameplay bar, never the visual one.
...
- **The First Night** is the title (D-015, 2026-07-22). **DRIFT** remains the internal codename for filenames, the repo, and crew shorthand — deliberately unchanged so no reference churns. The title appears in-game (cold-open card, page title) and on this charter.
