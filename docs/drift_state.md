...
- Current canon: **charter v0.4 · ops v1.4** (crew reorganized this cycle, D-044), plus this state, the decisions log, the cycle log, and the codex. All six live at `/docs/`; superseded versions in `/docs/archive/`, reference material (including the four handoff packages) in `/docs/reference/`.
...
2. **SON** — triage the reactions (FIX / TUNE / NEXT / PARK); run PERFECT with C2; then draft the **Cycle 05 — construction** spec + handoff package (D-040/D-037: shelter tiers, the bed as a real respawn anchor, energy/sleep, storage, upkeep — the cycle that finally needs real dynamics, so it is the Havok adoption trigger, D-031) and flag it to **C1** for MAJOR-artifact audit. **C1** renders the C04 close verdict from the KEY REPORT and resolves any forks (e.g. Havok adoption).
