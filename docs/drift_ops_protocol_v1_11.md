# DRIFT — Ops Protocol v1.11 *(project: The First Night)*
**Team · Process · Protocol** — the operating system for building DRIFT.

**Status:** standing. This file operationalizes Charter Part II. *(v1.1–v1.2, 2026-07-22 — dependency law §5.7; experience fields in §7. v1.3, 2026-07-22 — repo URL recorded in §8; §5 made engine-neutral for the 3D pivot, charter v0.4 §II.5. v1.4, 2026-07-23 — crew reorganization for token economy (D-044): SON Operator added, C1 becomes Supervisor, C2 default drops to Sonnet high with per-block/per-blockage Opus escalation; §1 crew table + supervision loop and §6 token economy revised. v1.5, 2026-07-23 — process-gap remediation (D-046): §4 gains four standing laws closing the gap the missing Cycle 05 OPEN spec exposed — no committed spec means no build, handoff packages embed their spec inline, MAJOR artifacts need C1's explicit "AUDITED — GO" relayed by the director, and a CI docs-integrity check keeps every decisions-log reference honest. v1.6, 2026-07-25 — supervision by waypoint (D-057, director): §3 gains the waypoint model (C1-blocking = constitution/forks/the Master Plan/Ch.9; dossier chapters and reconciliation pages move to non-blocking parallel review), and §4 gains the six escalation triggers and the long-session doctrine. **Routing changed, verification did not** — every gate, rail, and property test inside a build is untouched. v1.7, 2026-07-26 — chapter-numbering correction and a new hygiene law (D-060): the C1-blocking list reads "the Master Plan/Ch.10" after Ch.9 was corrected to "The Way Home" and the Master Plan (slicing) moved to Ch.10; §2 gains the rule that the state tracker is the single source of truth for chapter numbering and that a renumber is only real once its delta ships. **v1.8, 2026-07-27 — the Vacuity Law (D-066): §5 gains law 9, three clauses requiring that a test witness its target, that a defect-born regression be proven to fail pre-fix, and that a KEY REPORT name which audit legs genuinely ran.** Ratified after three vacuous passes shipped green — D-063's device-leg checks on a panel that never opened, D-064's property test whose guarded branch fired zero times across its whole sweep, and D-065's liveness check that would have passed on the very freeze it was written for. **Additive only** — the eight existing build laws and every gate in §3 are untouched. **v1.9, 2026-07-27 — bench isolation (D-072): §5 gains law 10.** The CPU-contention diagnosis carried since D-051 is retired, refuted by measurement (8% CPU; memory/buffer starvation on navigation). `preflight()`'s blanket `taskkill /F /IM chrome.exe` was itself the cause — a second harness killed the first's browser — so it now kills only what this run owns and **refuses to start while another run holds the bench lock**. **Additive only** — laws 1–9 and every gate in §3 are untouched. **v1.10, 2026-07-27 — §5 gains laws 11 and 12.** Law 11, standing hazard #4 (D-075): a debug hook driving a device check is not proof of a player path — named after experimentation shipped with no player entry point at all while its device checks passed by calling `__drift.tryCombine()`. Law 12, the Effectivity Law (D-076): every law declares whether it is OPERATIVE (in force, mechanism named) or DESIGN-BINDING (not yet live, trigger named); the unmarked middle is forbidden. **Additive only** — laws 1–10 and every gate in §3 are untouched. v1.11, 2026-07-28 — Vacuity law clause (d), "local is not live" (D-082): any claim that something is live must be witnessed against ORIGIN, never local HEAD; every session report cites the pushed SHA and an origin re-fetch. Ratified after Gate 0's close was reported landed across two sessions while origin sat eight commits behind — every local check green, and none of it visible to the crew reading the public URL. **Additive only** — clauses (a)–(c) and laws 1–12 are untouched.)* If it ever conflicts with the charter, the charter governs; the conflict is logged in the decisions log and reconciled next cycle.
...
**Chapter-numbering hygiene (D-060), added after a renumber was decided in chat and never shipped as a delta — canon then carried the stale number for three passes while everyone believed it had changed:**

1. **`drift_state.md`'s Dossier tracker is the SINGLE SOURCE OF TRUTH for chapter numbering.** Not the charter, not a decisions-log entry, not anyone's memory of a conversation. If the tracker and a chat message disagree, the tracker is right by definition.
2. **A renumber is only real once its delta ships in the same breath as the decree.** Intent expressed at the chat layer counts for nothing — a chapter is numbered what the tracker says it is numbered until a commit changes the tracker. Any ruling that renumbers a chapter must carry its own tracker edit, or it has not happened.

The same reasoning as law (b) in §4, applied to numbering rather than specs: **an artifact that only exists in conversation does not exist.**
...
### Supervision by waypoint (D-057)

The director holds a **Master Plan grant to SON**. Per-artifact "AUDITED — GO" (§4 law (c)) is **narrowed to C1-blocking items only** — it is no longer required for every MAJOR artifact in transit:

| Routing | What | Gate |
|---|---|---|
| **C1-blocking** | The constitution (charter/ops), forks, the Master Plan itself (Ch.10), and Ch.9 "The Way Home" | Still requires C1's explicit **"AUDITED — GO"** relayed by the director before BUILD |
| **Non-blocking, parallel review** | Dossier chapters and reconciliation pages | Go straight to BUILD; C1 reviews in parallel, and review findings land as normal FIX/TUNE/NEXT/PARK triage rather than a pre-build gate |

**Long-session doctrine.** A slice is built, self-verified, C3-audited, remediated, and closed with **ONE KEY REPORT** inside a single session, rather than surfacing at every stage boundary. The seven stages above still all happen and still have the same owners and outputs — they are simply no longer separate round-trips through the director for a non-blocking artifact.

**What this changes, stated precisely: routing, never verification.** Every gate, rail, done-check, and property test *inside* a build is untouched by this delta — the three definition-of-done gates above, the brain-purity check, the docs-integrity check, the offline-death-impossible law, Ch.2's amendment B (knowledge never decays offline), and the honest-systems rails all stand exactly as they did. This changes **who sees an artifact when**, not **what must be true before it ships**.
...
(c) **MAJOR artifacts flagged by SON reach C2 only with C1's explicit "AUDITED — GO"**, relayed by the director. A MAJOR artifact (cycle spec, handoff package, constitutional delta) in transit without that relay is not yet cleared for BUILD. **Narrowed by D-057 (v1.6):** this now applies to **C1-blocking items only** — the constitution, forks, the Master Plan (Ch.10), and Ch.9 "The Way Home". Dossier chapters and reconciliation pages proceed to BUILD without it, under C1's parallel (non-blocking) review; see §3's waypoint table. Laws (a), (b), and (d) are **unchanged and apply to every artifact regardless of routing** — a dossier chapter still may not be built from a spec that never travelled inline.
...
**The six escalation triggers (D-057).** Under the long-session doctrine, C2 runs a whole slice without stage-boundary check-ins — so the conditions that DO warrant stopping mid-slice are enumerated rather than left to judgment. Any one of these means: stop, state the problem plainly, and escalate rather than ship a weakened version.

1. **Law / property-test contradiction** — a slice's requirements cannot be satisfied without violating a standing law or weakening an existing property test (the offline-death-impossible law, Ch.2's amendment B, or any successor).
2. **Scope beyond the current slice** — the work as specified cannot be completed without building something the slice does not name.
3. **Identity, tone, arc, or business-posture questions** — anything touching what the game *is* or how it presents itself, rather than how it works.
4. **An architecture wall past two attempts** — two genuine attempts at a structural problem have failed. (Flag it, escalate the tier, then flip back.)
5. **Honest-systems or safety doubt** — any doubt that what is being built could mislead a player about real game state, or is otherwise unsafe.
6. **Genuine uncertainty** — the plain case: not knowing, where guessing would be worse than asking.
...
9. **Vacuity law** (D-066, extended by [[D-082]]) — *"a test must witness its target."* Four clauses, all binding on C2 and checkable by C3:
   - **(a) Witness the branch.** Every property test must assert that its guarded branch actually fired — either a minimum hit count across the tested corpus, or constructed cases that force it. **A branch that never fires FAILS the test as vacuous; it does not pass by default.**
   - **(b) Prove the fail first.** Every regression test born from a live, reported defect must be proven to **FAIL against the pre-fix tree** before it is trusted. (D-065's A/B practice, now law: check the pre-fix state out whole, run the new test against it, and only then believe a green.)
   - **(c) Name the legs.** Every KEY REPORT states explicitly, **per package**, which audit legs genuinely ran — static, unit, device — with **no leg implied that did not happen.** "Audited" without a named leg is not a claim, it is a gap.
   - **(d) Local is not live.** ([[D-082]]) Any claim that a commit, document, or ledger entry is **live**, **landed**, or **applied** must be witnessed against **ORIGIN** — the public raw URL or the API — **never local HEAD**. Every docs or build session report cites the **pushed SHA** and an **origin re-fetch confirmation**. A clean working tree, a green `docs-integrity` and a correct local file prove only that the machine under your hands is correct; they say nothing about what the crew can read. Gate 0's close was reported landed across two sessions while origin sat eight commits behind. **A report naming no SHA and quoting no re-fetch is making no claim at all.** The raw URL is **CDN-cached and lags origin by minutes**, so it is not sufficient alone: pair it with `git ls-remote origin main` or the API, and report all paths when they disagree. A raw URL behind a push means *not yet propagated*, not *not pushed* — but note it can only ever show something **older** than origin, never newer, so a stale cache can never manufacture a false "this is live".

   This is tracked as a **standing hazard**, alongside worktree destruction — not absorbed per-occurrence and forgotten.

10. **Bench isolation law** (D-072) — *"separate ports are not isolation; separate times are."*
    - **One harness at a time.** A run takes a bench lock; a second run **refuses to start** rather than beginning and silently corrupting the first. Serialize.
    - **Kill only what you own.** No blanket process kills. A run tears down the browser it launched, on every exit path, and touches nothing else. The old blanket `taskkill /F /IM chrome.exe` is what made separate ports useless as isolation.
    - **Do not diagnose a crash as "contention" without measuring it.** This project blamed CPU contention for four sessions; the measurement was 8% CPU with memory/buffer starvation on navigation. A wrong cause that excuses a failure is worse than no cause — it retires the question.

    Standing hazard **#3**, tracked alongside worktree destruction (#1) and vacuous verification (#2).

11. **Player-path law** (D-075) — *"a debug hook driving a device check is not proof of a player path."*
    - Every player-facing capability is device-verified **through the player's own route**: real input on the real visible control, to D-053's `isVisible` standard (computed style + in-viewport rect + topmost hit-test).
    - **Debug hooks may READ state. They may never DRIVE path verification.** `__drift.x()` answering correctly proves the brain works and says nothing about whether a human can reach it.
    - Standing hazard **#4**. Named after experimentation shipped across two packages with **no player entry point whatsoever**, its device checks green the whole time because they called the hook.

12. **Effectivity law** (D-076) — *"no claim without a witness."*
    - Every ledger law and amendment declares its class: **OPERATIVE** (in force against the live game; satisfying mechanism NAMED, and either already shipped or landing in the same batch) or **DESIGN-BINDING** (governs design; trigger or slice NAMED; explicitly not-yet-live).
    - **A live-game guarantee with no shipped witness is illegal.** The forbidden class is the unmarked middle — law-shaped prose that claims the present with no witness.
    - Applies **from ratification forward only**; the existing ledger is not audited against it.
    - This law's own witness is the docs-integrity check plus C3's audit reading.

    Laws 9, 11 and 12 are one discipline in three places: **tests witness code, laws witness mechanisms, reports witness their own audit legs.**
